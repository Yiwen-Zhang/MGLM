...
## Set class
##============================================================##
setClass("MGLMsparsereg", representation(
				call="function",
				data = "list",
				coefficients="matrix",
				logL="numeric",
...
MGLMsparsereg<- function (formula, data, dist, lambda, penalty, weight, init, 
    penidx, maxiters = 150, ridgedelta, epsilon = 1e-05, regBeta = FALSE, 
    overdisp) 
{
    call <- match.call()
    if (missing(data)) 
    ...
    if (missing(ridgedelta)) 
        ridgedelta <- 1/(p * d)
    est <- eval(call("MGLMsparsereg.fit", Y = Y, X = X, dist = dist, 
        lambda = lambda, penalty = penalty, weight = weight, 
        init = init, penidx = penidx, maxiters = maxiters, ridgedelta = ridgedelta, 
        epsilon = epsilon, regBeta = regBeta, overdisp = overdisp))
    est$call <- match.call()
    est$data <- list(Y = Y, X = X)
    est$distribution <- ifelse(dist == "MN", "Multinomial", ifelse(dist == 
        "DM", "Dirichlet Multinomial", ifelse(dist == "GDM", 
        "Generalized Dirichlet Multinomial", "Negative Multinomial")))
    est$penalty <- ifelse(penalty == "group_row", "group", ifelse(penalty == 
        "nuclear", "nuclear", "sweep"))
    est$lambda <- lambda
...
MGLMsparsereg.fit <- function (Y, X, dist, lambda, penalty, weight, init, penidx, 
    maxiters = 150, ridgedelta, epsilon = 1e-05, regBeta = FALSE, 
    overdisp) 
{
    ...
    while ((niter < 3) || (!stop)) {
        ...
        for (l in 1:50) {
            ...
            pen <- matrix_threshold(X = Apen, lambda = ridgedelta * 
                lambda, penalty = penalty)
            B[penidx, ] <- pen[[1]]
            penval <- pen[[2]]
            ...
            objloss <- MGLM.loss(Y, X, B, dist, weight, regBeta, 
                overdisp)
            loss.S2 <- objloss[[1]]
            loss.D1S2 <- objloss[[2]]
            objval <- loss.S2 + penval
            BminusS <- B - S
            surval <- loss.S + sum(loss.D1S * BminusS) + norm(BminusS, 
                type = "F")^2/2/ridgedelta + penval
            if (!is.na(objval) && !is.na(surval)) 
                if (objval <= surval) 
                  break
                else ridgedelta <- ridgedelta/2
        }
        alpha_old <- alpha
        alpha <- (1 + sqrt(4 + alpha_old^2))/2
        if (!is.na(objval) & objval <= obj1) {
            stop <- abs(obj1 - objval) < epsilon * (abs(obj1) + 
                1)
            ...
            if (isdescent) {
                isdescent <- FALSE
                stop <- FALSE
                S <- B + (alpha_old - 1)/alpha * (beta_old - 
                  B)
                B <- beta_old
            }
            else {
    ...
    if (penalty == "sweep") {
        ...
        Dof <- sum(!penidx) * ncol(B) + sum(rowSums(Bpen^2) != 
            0) + sum((ncol(B) - 1) * rowSums(Bpen^2)/rowSums(Apen^2))
    }
    else if (penalty == "nuclear") {
        ...
        if (sum(Aspectrum > ridgedelta * lambda) == 0) {
            ...
            for (i in 1:sum(Aspectrum > ridgedelta * lambda)) {
                Dof <- Dof + 1 + 2 * sum(Aspectrum[i] * (Aspectrum[i] - 
                  ridgedelta * lambda)/(Aspectrum[i]^2 - Aspectrum[-i]^2)) + 
                  abs(p - d) * (1 - ridgedelta * lambda/Aspectrum[i])
            }
    ...
    return(list(coefficients = B, logL = logL, AIC = AIC, BIC = BIC, 
        Dof = Dof, iter = niter, maxlambda = maxlambda))
}
...
MGLM.loss <- function (Y, X, beta, dist, weight, regBeta = FALSE, Beta) 
{
    ...
    if (dist == "MN") {
        ...
        lossD1 <- -rowSums(sapply(1:nrow(Y), function(i, A, B, 
            w) return(w[i] * A[i, ] %x% B[i, ]), alpha * dalpha, 
            X, weight))
        lossD1 <- matrix(lossD1, p, d)
    }
    else if (dist == "GDM") {
        Ys <- t(apply(apply(apply(Y, 1, rev), 2, cumsum), 2, 
            rev))
        alpha <- exp(X %*% beta)
        A <- alpha[, 1:(d - 1)]
        B <- alpha[, d:(2 * (d - 1))]
        loss <- -sum(weight * dgdirm(Y, A, B))
        dalpha1 <- digamma(A + Y[, -d]) - digamma(A) - digamma(A + 
            B + Ys[, -d]) + digamma(A + B)
        dalpha2 <- digamma(B + Ys[, -1]) - digamma(B) - digamma(A + 
            B + Ys[, -d]) + digamma(A + B)
        ...
        if (regBeta) {
            ...
            deta[, 1:d] <- Y - alpha[, 1:d] * Beta - P[, 1:d] * 
                (m - (alpha_rowsums - 1) * Beta)
            deta[, (d + 1)] <- Beta * (digamma(Beta + m) - digamma(Beta) + 
                log(P[, (d + 1)]))
            lossD1 <- -rowSums(sapply(1:nrow(Y), function(i, 
                A, B, w) return(w[i] * A[i, ] %x% B[i, ]), deta, 
                X, weight))
            lossD1 <- matrix(lossD1, p, (d + 1))
        }
        else {
            ...
            deta <- Y - (Beta + m) * P[, 1:d]
            lossD1 <- -rowSums(sapply(1:N, function(i, A, B, 
                w) return(w[i] * A[i, ] %x% B[i, ]), deta, X, 
                weight))
            lossD1 <- matrix(lossD1, p, d)
        }
    }
...
##============================================================================##
## lsq_threshold
## We can only work on lasso for now
##============================================================================##
lsq_thresholding <- function(b, lambda){
	if(lambda <0) stop("penalty constant lambda should be nonnegative")
