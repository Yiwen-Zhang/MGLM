...
## set class
##============================================================##
setClass("MGLMreg", representation(
				call="function",
				data = "list",
				coefficients="list",
				SE= "list",
...
MGLMreg <- function(formula, data, dist, init, weight, 
					epsilon=1e-8 ,maxiters=150, display=FALSE, 
					LRT=FALSE, parallel=FALSE, cores, regBeta=FALSE){
	...
	if(nrow(X) !=N)	stop("Unequal numbers of observations in X and Y")
...
#     		init <- rbind(rep(init[1:(d-1)],2), matrix(0, (p-1),2*(d-1)))
	...
	if(dist=="MN"){
		...
		colnames(est$SE) <- c(paste("alpha", colnames(Y)[1:(d-1)], sep=""),
				paste("beta_", colnames(Y)[1:(d-1)], sep="") )	
	...
	logL <- est$logL
 	if(LRT){
		...
		for(t in 1:p){
			subX <- X[, -t] 
			if(dist=="MN"){
			subest <- eval(call("DMD.MN.reg", 
				Y=Y, X=X[,-t], weight=weight, init=init[-t,], 
				epsilon=epsilon, maxiters=maxiters, display=FALSE,
				parallel=FALSE, cores=cores, cl=cl, sys=sys))	   
			}else if(dist=="DM"){
			subest <- eval(call("DMD.DM.reg", 
				Y=Y, X=X[,-t], weight=weight, init=init[-t,], 
				epsilon=epsilon, maxiters=maxiters, display=FALSE,
				parallel=FALSE, cores=cores, cl=cl, sys=sys))	
			}else if(dist=="NegMN"&regBeta){
			subest <- eval(call("DMD.NegMN.reg", 
				Y=Y, X=X[,-t], weight=weight, init=init[-t,], 
				epsilon=epsilon, maxiters=maxiters, display=FALSE,
				parallel=FALSE, cores=cores, cl=cl, sys=sys))	
			}else if(dist=="NegMN"&(!regBeta)){
			subest <- eval(call("DMD.NegMN.Alpha.reg", 
				Y=Y,  X=X[,-t], weight=weight, init=init[-t,], 
				epsilon=epsilon, maxiters=maxiters, display=FALSE,
				parallel=FALSE, cores=cores, cl=cl, sys=sys))
			}else if(dist=="GDM"){
			subest <- eval(call("DMD.GDM.reg", 
				Y=Y, X=subX, weight=weight, init=init[-p,], 
				epsilon=epsilon, maxiters=maxiters, display=FALSE,
				parallel=FALSE, cores=cores, cl=cl, sys=sys))	
		   }
		   sublogL <- subest$logL
	...
	est$distribution <- ifelse(dist=="MN", "Multinomial", 
...
##============================================================##
##	Function 1 fit Multinomial Reg
##
## The dimension of Y is n by d (d>=3)
##============================================================##

DMD.MN.reg <- function(Y,init,  X , weight, 
		epsilon, maxiters, display, 
	...
	H.MN_fctn <- function(i, beta){
		...
		y <- Y[i,]
		prob <- exp( colSums(x*beta) )/(sum( exp(colSums(x*beta)) )+1)
    if(d>2) dprob <- diag(prob) else if(d==2) dprob <- prob
...
#		dl <- Reduce("+", lapply(1:N, dl.MN_fctn, beta) )
		...
		if(is.null(update)){
			...
			if(is.nan(ll.Newton)|ll.Newton >=0) ll.Newton <- NA
		}else{ ll.Newton <- NA }
		...
		if(is.na(ll.Newton) || ll.Newton <ll1){
			...
			if(!parallel){
				for(i in 1:(d-1)){
					beta_MM[,i] <- glm.fit(X,Y[,i]/a, weights=weight*a, 
				...
				y.list[[d]] <- NULL
				if(sys=="Windows"){
					fit.list <- clusterMap(cl, glm.private, y.list, 
							.scheduling="dynamic", MoreArgs=list(weights=weight*a, X=X, 
							family=poisson(link ="log")))
				}else if(sys!="Windows"){
					fit.list <- mcmapply(cl, glm.private, y.list, 
							MoreArgs=list(weights=weight*a, X=X, 
							family=poisson(link ="log")), 
			...
			if( is.na(ll.Newton)|(ll.MM<0&ll.MM>ll1)){
				if(display) print(paste("Iteration ", niter, " MM update", ll.MM, sep=""))
				beta <- beta_MM
				lliter[niter] <- ll.MM 
				ll2 <- ll.MM
			}
		}else{
			if(display) print(paste("Iteration ", niter, " Newton's update", ll.Newton, sep=""))
			beta <- beta_Newton
...
objfun <- function(alpha, x, y, d, p){
  ...
  logl <- rowSums(lgamma(y+alpha)-
                    lgamma(alpha))+lgamma(rowSums(alpha))-lgamma(rowSums(alpha)+m)+
...
objfun.hessian <- function(alpha, x, y, d, p){
  ...
  for(i in 1:d){
    ...
    Hessian[idx, idx] <- Hessian[idx, idx] - 
      t(x) %*% ((tmpvector*Beta[,i]- tmpmatrix2[,i])*x)
  }
  return(-Hessian)
}


DMD.DM.reg <- function( Y, init, X, weight,
                        epsilon, maxiters, display, parallel, cores, cl, sys){
...
#     dl <- colSums( kr(dalpha, X, weight) )
    ...
    if(all(!is.na(dl)) & mean(dl^2) < 1e-4) break
    temp.try <- NULL
   ...
   if( is.na(ll.Newton)|| ll.Newton<ll1 ){
      ...
      if(!parallel){
        for(j in 1:d ){ 
          ...
          ff <- glm.fit(X1, Y_new[,j], weights=weight.fit, 
                        family=poisson(link="log"),control=list(epsilon = 1e-8))          
#    		  a <- b <- NA
#         ff <- nlminb(rep(0.1, p), ll.obj, gradient=ll.grad, #hessian=ll.hessian, 
#                        y=wy, X=X1, w=weight.fit)
#         b <- -ff$objective
#         a <- -ll.obj(beta[,j], wy, X1, weight.fit)
#         b <- -ll.obj(ff$coefficients, wy, X1, weight.fit)
  		  if(ff$converged) beta_MM[,j] <- ff$coefficients #par
#       print(paste( ff$converged, sep="  ***  "))
      ...
      ## Choose the update
      if(is.na(ll.Newton)|(ll.MM<0&ll.MM>ll1)){
        if(display) print(paste("Iteration ", niter,  " MM update, log-likelihood ", ll.MM, sep=""))
        beta <- beta_MM
        Beta <- exp(X%*%beta_MM)
        lliter[niter] <- ll.MM 
        ll2 <- ll.MM
  ...
  if(any(Beta==Inf, is.nan(Beta), is.nan(tmpvector2), 
    ...
    if(mean(dl^2)> 1e-4){
      warning(paste("The algorithm doesn't converge within", niter,
                    "iterations. The norm of the gradient is", sqrt(sum(dl^2)), 
    ...
    eig <- eigen(H)$values
    if(any(is.complex(eig)) || any(eig>0) || any(abs(eig)<1e-4) ){
...
DMD.GDM.reg <- function(Y,init,X,weight,epsilon,maxiters, display, 
  ...
  if(is.null(init)){
	...
	if(!parallel){
		...
		if(sys[1]=="Windows"){
			fit.list <- clusterMap(cl, DMD.DM.reg, Y.list, init.list,
				.scheduling="dynamic",
				MoreArgs=list(X=X, weight=weight, epsilon=epsilon, 
				maxiters=maxiters, display=display, parallel=FALSE))
		}else if(sys[1]!="Windows"){
			fit.list <- mcmapply(cl, DMD.DM.reg, Y.list,init.list,
				MoreArgs=list(X=X, weight=weight, epsilon=epsilon, 
				maxiters=maxiters, display=display, parallel=FALSE),
				mc.cores = cores, mc.preschedule = FALSE)
		}
	...
	B1 <- exp(X%*%alpha)
	B2 <- exp(X%*%beta)
	a1 <- B1*(digamma(B1+Y[,-d])-digamma(B1))-
			B1^2*(-trigamma(B1+Y[, -d])+trigamma(B1) )
	a2 <- B1*(digamma(B1+B2+Ys[,-d])-digamma(B1+B2)) - 
			B1^2*(-trigamma(B1+B2+Ys[, -d])+trigamma(B1+B2))
	b <- B1*B2*(-trigamma(B1+B2+Ys[,-d]) + trigamma(B1+B2))
	d1 <- B2*(digamma(B2+Ys[,-1])-digamma(B2))-
	...
	if(any(is.nan(a1), is.nan(a2), is.nan(b), is.nan(d1), is.nan(d2))){
      ...
      Hinv <- solve(-H)
			...
			for(j in 1:p){
				id<- c(0: (2*(d-1)-1))*p+j	
				invH <- NULL
				try(invH <- solve(Hinv[id,id], c(alpha[j,] ,beta[j,])), 
						silent=TRUE)
				if(is.numeric(invH)){
	...
	fitted[,1]<- exp(Xf%*%alpha[,1])/(exp(Xf%*%alpha[,1])+exp(Xf%*%beta[,1]))
	fitted[,2] <- (1-fitted[,1])*exp(Xf%*%alpha[,2])/
			(exp(Xf%*%alpha[,2])+exp(Xf%*%beta[,2]))
  if(d>3){
  	for(f in 3:(d-1)){
  		fitted[,f] <- (1-rowSums(fitted[,1:(f-1)]))*exp(Xf%*%alpha[,f])/
  				(exp(Xf%*%alpha[,f])+exp(Xf%*%beta[,f]))
  	}
  }else if(d==3){
	...
	list(coefficients=cbind(alpha[, outOrder],beta[, outOrder]), 
      ...
      Hessian=H[c(outOrder, outOrder+d-1),c(outOrder, outOrder+d-1)], 
			...
			gradients=gradients, fitted=fitted[,order(colOrder)])
                           ...
                           cores, cl, sys) 
{
  ...
  niter <- 1
  options(warn = -1)
  while (((niter <= 2) || ((ll2 - ll1)/(abs(ll1) + 1) > epsilon)) & 
           (niter < maxiters)) {
    niter <- niter + 1
    ll1 <- lliter[niter - 1]
    Alpha <- exp(X %*% alpha)
    ...
    hbeta_w <- (trigamma(Beta + m) - trigamma(Beta) + tmpBeta - 
                  w_beta) * Beta
    hbeta <- sapply(1:N, function(i, A, B, w) return(w[i] * 
                                                       A[i, ] %x% B[i, ]), X, X, hbeta_w)
    ...
    if (all(eigen(hbeta)$value < 0)) {
      ...
      if (lltemp < ll1) {
        ...
        beta_MM <- glm.fit(X, Y_reg, weights = w_beta, 
                           family = poisson(link = "log"), start = beta)$coefficients
      }
    }
    else {
      Y_reg <- weight * Beta * tmpBeta/w_beta
    ...
    if (!parallel) {
      for (j in 1:d) {
        alpha_MM[, j] <- glm.fit(X, weight * Y[, j]/w_alpha, 
                                 weights = w_alpha, family = poisson(link = "log"), 
                                 start = alpha[, j])$coefficients
      }
    }
    else {
      ...
      if (sys[1] == "Windows") {
        fit.list <- clusterMap(cl, glm.private, y.list, 
                               start.list, .scheduling = "dynamic", MoreArgs = list(X = X, 
                                                                                    family = poisson(link = "log"), weights = w_alpha))
      }
      else if (sys[1] != "Windows") {
        fit.list <- mcmapply(cl, glm.private, y.list, 
                             start.list, MoreArgs = list(X = X, family = poisson(link = "log"), 
                                                         weights = w_alpha), mc.cores = cores, mc.preschedule = FALSE)
      }
      alpha_MM <- do.call("cbind", fit.list)
    }
    Alpha_MM <- exp(X %*% alpha_MM)
    Prob_MM <- cbind(Alpha_MM/(1 + rowSums(Alpha_MM)), 1/(1 + 
                                                            rowSums(Alpha_MM)))
    Beta_MM <- c(exp(X %*% beta_MM))
    ll.MM <- sum(weight * dneg(Y, Alpha_MM, Beta_MM), na.rm = TRUE)
    deta <- matrix(0, N, (d + 1))
    deta[, 1:d] <- Y - Alpha * Beta - Prob[, 1:d] * (m - 
                                                       Beta * (rowsum_Alpha - 1))
    ...
    upleft <- sapply(1:N, function(i, A, B) return(A[i, ] %x% 
                                                     B[i, ]), cbind(Prob[, 1:d], -Beta/(Beta + m)), X)
    hessian <- t(t(upleft) * (weight * (Beta + m))) %*% t(upleft)
    for (j in 1:d) {
      idx <- (j - 1) * p + (1:p)
      hessian[idx, idx] <- hessian[idx, idx] - t(X) %*% 
        (X * (weight * (Beta + m) * Prob[, j]))
    }
    tmpvector2 <- as.vector(Beta * (tmpBeta + Beta * (trigamma(Beta + 
                                                                 m) - trigamma(Beta)) - log(rowsum_Alpha) - Beta/(Beta + 
                                                                                                                    m)))
    idx <- d * p + c(1:p)
    hessian[idx, idx] = hessian[idx, idx] + t(X) %*% (X * 
                                                        (weight * tmpvector2))
    temp.try <- NULL
    ...
    if (is.null(temp.try)) {
      ...
      B_Newton <- cbind(alpha, beta) - matrix(temp.try, 
                                              p, (d + 1))
      B <- exp(X %*% B_Newton)
      Alpha_Newton <- B[, 1:d]
      Beta_Newton <- B[, (d + 1)]
      ll.Newton <- sum(weight * dneg(Y, Alpha_Newton, Beta_Newton), 
                       na.rm = TRUE)
      if (!is.na(ll.Newton) & ll.MM > ll.Newton) {
        for (step in 1:20) {
          B_N <- cbind(alpha, beta) - matrix(temp.try/(2^step), 
                                             p, d + 1)
          B <- exp(X %*% B_N)
          Alpha_N <- B[, 1:d]
          Beta_N <- B[, (d + 1)]
          llnew <- sum(weight * dneg(Y, Alpha_N, Beta_N), 
                       na.rm = T)
          if(is.na(llnew)|is.nan(llnew) | llnew>0){
            next
          }else if(llnew > ll.Newton){
    ...
    if (is.na(ll.Newton) | ll.MM > ll.Newton) {
      if (display) 
        print(paste("Iteration ", niter, " MM update", 
                    sep = ""))
      alpha <- alpha_MM
      beta <- beta_MM
      ll2 <- ll.MM
      ...
      if (display) 
        print(paste("Iteration ", niter, " Newton's update", 
                    sep = ""))
      alpha <- B_Newton[, 1:d]
      beta <- B_Newton[, (d + 1)]
      ll2 <- ll.Newton
  ...
  if (any(A == Inf, is.nan(A))) {
    ...
    score <- rowSums(sapply(1:N, function(i, A, B, weight) return(weight[i] * 
                                                                    A[i, ] %x% B[i, ]), deta, X, weight))
    H <- sapply(1:N, function(i, a, x) return(a[i, ] %x% 
                                                x[i, ]), cbind(Prob[, 1:d], -Beta/tmpv2), X)
    H <- H %*% (weight * tmpv2 * t(H))
    for (i in 1:d) {
      id <- (i - 1) * p + c(1:p)
      H[id, id] <- H[id, id] - t(X) %*% (weight * tmpv2 * 
                                           Prob[, i] * X)
    }
    id <- d * p + c(1:p)
    tmpv3 <- Beta * (tmpv1 + Beta * (trigamma(tmpv2) - trigamma(Beta)) + 
                       log(Prob[, (d + 1)]) - Beta/tmpv2)
    H[id, id] <- H[id, id] + t(X) %*% (weight * tmpv3 * X)
    if (mean(score^2) > 1e-04) {
      warning(paste("The algorithm doesn't converge within", 
                    niter, "iterations. The norm of the gradient is ", 
    ...
    if (any(eig > 0)) {
      ...
      for (j in 1:p) {
        id <- c(0:d) * p + j
        wald[j] <- c(alpha[j, ], beta[j]) %*% chol2inv(chol(Hinv[id, 
                                                                 id])) %*% c(alpha[j, ], beta[j])
        wald.p[j] <- pchisq(wald[j], (d + 1), lower.tail = FALSE)
      }
    }
  }
  fitted <- c(exp(X %*% beta)) * exp(X %*% alpha)/rowSums(exp(X %*% 
                                                                alpha))
  list(coefficients = cbind(alpha, beta), SE = SE, Hessian = H, 
       BIC = BIC, AIC = AIC, wald.value = wald, wald.p = wald.p, 
       logL = lliter[niter], iter = (niter), gradient = score, 
       fitted = fitted)
}

# DMD.NegMN.reg <- function(Y, init, X, weight, 
# 		epsilon, maxiters, display, parallel, cores, cl, sys){
# 
#   ##----------------------------------------##
#   ## 	Keep some original values
...
# 			}else if(!is.na(ll.Newton)&ll1 >= ll.Newton){
# 				for(step in 1:40){
# 					B_N <- cbind(alpha, beta) - matrix(temp.try*(0.5^step), p, d+1)
# 					B <- exp(X%*%B_N)
# 					Alpha_N <- B[, 1:d] 
# 					Beta_N <- B[, (d+1)]
# 					llnew <- sum(weight*dneg(Y, Alpha_N, Beta_N),na.rm=T)
# 					if(is.nan(llnew) | is.na(llnew) | llnew>0){
# 						next
# 					}else if(llnew > ll.Newton){
# 						ll.Newton <- llnew
# 						B_Newton <- B_N
...
# 				}
# 			}
# 			if(is.nan(ll.Newton)) ll.Newton <- NA
# 			else if(ll.Newton>=0) ll.Newton <- NA
# 		}else{ ll.Newton <- NA }
# 		
# 		if(is.na(ll.Newton) || ll.Newton<ll1){
# 			## ----------------------------------------##
...
# 				start.list <- split(alpha, rep(1:d, each=nrow(alpha)))
# 				if(sys[1]=="Windows"){
# 				fit.list <- clusterMap(cl, glm.private, y.list, start.list, 
# 					.scheduling="dynamic",
# 					MoreArgs=list(X=X, family=poisson(link ="log"), weights=w_alpha))
# 				}else if(sys[1]!="Windows"){
# 				fit.list <- mcmapply(cl, glm.private, y.list, start.list, 
# 					MoreArgs=list(X=X, family=poisson(link ="log"), weights=w_alpha),
# 					mc.cores = cores, mc.preschedule = FALSE)
# 				}
# 				alpha_MM <- do.call("cbind", fit.list)
...
# 		id <- d*p+c(1:p)
# 		tmpv3 <- Beta*(
# 					tmpv1+ Beta*(trigamma(tmpv2)-trigamma(Beta))+
# 					log(Prob[, (d+1)])-Beta/tmpv2)
# 		H[id, id] <- H[id, id]+t(X)%*%(weight*tmpv3*X)
# 
# 		##----------------------------------------##
...
DMD.NegMN.Alpha.reg <- function(Y, init, X, weight, 
		epsilon, maxiters, display, parallel, cores, cl, sys){
	...
	while( ((niter <=2)|| ((ll2-ll1)/(abs(ll1)+1) > epsilon))&(niter<maxiters) ){
		...
		if(is.null(temp.try)){
			...
			ll.Newton <- sum(weight*dneg(Y,Alpha_Newton, rep(beta_Newton,N)), na.rm=TRUE)
		}
		##----------------------------------------##
		## Half stepping
		if(is.nan(ll.Newton) || ll.Newton>=0){
		...
		if( is.na(ll.Newton) || ll.Newton<ll1 ){
			...
			if( is.na(ll.Newton)|ll.MM>ll1){
				if(beta_MM<=0){
				  warning(paste("The estimate of overdispersion parameter is ,", beta, 
								". It is smaller or eqal to zero.  Please consider other models.", sep=" "))
				  div <- TRUE
				  break
				}
				beta <- beta_MM
			...
			lliter[niter] <- ll.Newton
			ll2 <- ll.Newton 
			if(display) print(paste("Iteration ", niter,  " Newton's update, log-likelihood ", ll.Newton, sep=""))		
		}
		if(div) break
	}
	##----------------------------------------##
