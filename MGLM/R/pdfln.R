# Functions to calculate log of pdf of each of the multivariate
# models. 
# 
# Author: Yiwen Zhang
###############################################################################
...
dgdirm <- function(Y, alpha, beta){
	...
	logl <- lgamma(m+1) - rowSums(lgamma(Y+1)) +
			rowSums(lgamma(Y[,-d] + alpha)) - rowSums(lgamma(alpha))+
			rowSums(lgamma(z[, -1]+beta) ) - rowSums(lgamma(beta)) - 
			rowSums(lgamma(alpha + beta + z[,-d])) + 
			rowSums(lgamma(alpha + beta))
...
dnegmn <- function(Y, prob, beta){
	...
	m <- rowSums(Y)
	d <- ncol(Y)
	logl <- lgamma(beta + rowSums(Y)) -lgamma(beta)-rowSums(lgamma(Y+1)) + 
			rowSums(Y*log(prob)) + beta*log(1-rowSums(prob))	
 	return(logl)
}
...
dneg <- function(Y, alpha, beta){
	...
	d <- ncol(Y)
	logl <- lgamma(beta + rowSums(Y)) -lgamma(beta)-rowSums(lgamma(Y+1)) + 
			rowSums(Y*log(alpha)) - (beta+m)*log(rowSums(alpha)+1)
