...
## Set class
##============================================================##
setClass("MGLMfit", representation(
		estimate="vector", 
		SE="vector",
		vcov="matrix",
		logL="numeric",
...
MGLMfit <- function( data, dist, init, weight, 
	...
	if( any(colSums(data)==0) ){ 
		...
		warning(
			paste(rmv, " columns are removed because the column sums are 0", sep=""))
	}
	}
	...
	if(dist=="DM"){
		...
		if(!is.null(colnames(data))){
			names(est$estimate) <-c(
				paste('alpha', colnames(data)[1:(d-1)], sep="_"), 
				paste('beta', colnames(data)[1:(d-1)], sep="_"))
		}else{
			names(est$estimate) <-c(
				paste('alpha', 1:(d-1), sep="_"), 
				paste('beta', 1:(d-1), sep="_"))
		}
	}else if(dist=="NegMN"){
		if(!is.null(colnames(data))){
			names(est$estimate) <- c(
					paste("p", colnames(data), sep="_"), "phi")
		}else{
			names(est$estimate) <- c(paste("p", 1:d, sep="_"),"phi")
		}
	}

	est$distribution <- ifelse(dist=="DM", "Dirichlet Multinomial",
...
DMD.DM.fit <- function( data, init, weight, 
			epsilon=1e-8, maxiters=150, display=FALSE){

	##----------------------------------------##
	## Remove the zero rows and columns
	if(!missing(weight)) weight <- weight[rowSums(data)!=0]
	...
	LL <- -sum(r*log(sum(alpha_hat)+k)) + sum(s*log(outer(k,alpha_hat,"+")))+log_term
	alpha_hat <- init	
	...
	## The main loop
	while( ((niter<=2) || 
				((DM.LL2-DM.LL1)/(abs(DM.LL1)+1)>epsilon))&(niter<maxiters) ){
		
		niter <- niter+1
		DM.LL1 <- -sum(r*log(sum(alpha_hat)+k))+
				sum(s*log(outer(k,alpha_hat,"+")))+log_term

		##----------------------------------------##
		## MM update
		numerator <- colSums( s/(outer(rep(1,length(k)),alpha_hat)+
				outer(k,rep(1,d))) )
		...
		dl <- ( colSums( s/(outer(rep(1,max),alpha_hat)+
			outer(k,rep(1,d))) )- sum( r/(sum(alpha_hat)+k) ) )
		alpha_Newton <- alpha_hat+(1/b)*dl+( a*sum((1/b)*dl)/(1-a*sum(1/b)) )*(1/b)
		...
		LL.Newton <- -sum(r*log(sum(alpha_Newton)+k))+
				sum(s*log(outer(k,alpha_Newton,"+")))+log_term
		if( LL.MM > LL.Newton ){
			alpha_hat <- alpha_MM
			if(display) print(paste("Iteration", niter, "MM update", sep=" "))
		}else{
			alpha_hat <- alpha_Newton
			if(display) print(paste("Iteration", niter, "Newton update", sep=" "))
		}
		}
		DM.LL2 <- -sum(r*log(sum(alpha_hat)+k))+
				sum(s*log(outer(k,alpha_hat,"+")))+log_term
	...
	## 1)SE 2) LRT test against the MN model 3) BIC
	invI <- diag(1/b)+ a/(1-a*sum(1/b))*outer(1/b,1/b,"*")
	SE <- sqrt(diag(invI))
	p_MN <- t(matrix(apply( data/m, 2, mean),d, N))
	logL_MN <- sum( weight*data*log(p_MN))+sum(lgamma(m+1))-sum(lgamma(data+1))
	...
	list(estimate=alpha_hat, SE=SE, vcov = invI, gradient=dl,
			logL=DM.LL2, iter=niter, BIC=BIC, AIC=AIC, LRT=LRT,
			LRTpvalue=p_value, fitted=fitted, DoF=DoF)
}
...
DMD.GDM.fit <- function(data,init,weight,
				epsilon=1e-8, maxiters=150, display=FALSE){
	...
	list(estimate=c(alpha_hat,beta_hat), SE=c(SE), gradient=gradients,
		logL=sum(LL), BIC=BIC, AIC=AIC, iter=sum(niter), LRT=LRT, 
		LRTpvalue=p_value, fitted=fitted, DoF=DoF)
}
...
DMD.NegMN.fit <- function( data, init, weight, 
				epsilon=1e-8,maxiters=150, display=FALSE){
	...
	pi_hat <- init_pi[1:d]
	pi2_hat <- init_pi[(d+1)]
	LL1 <- sum(lgamma(beta_hat+m))+ sum(data%*%log(pi_hat))+ 
			N*beta_hat*log(pi2_hat)- sum(lgamma(data+1))
    ...
    LL1 <- LL_iter[niter]
		...
		pi_MM <- Xj/(sum(Xj)+N*beta_MM)
 		LL_MM <- sum(r*log(beta_MM+k))+ sum(Xj*log(pi_MM))+ 
 			N*beta_MM*log(pi2_MM)- sum(lgamma(data+1))

		##----------------------------------------##
		## Newton's Update
		score <- c( Xj/pi_hat - sum(weight)*beta_hat/pi2_hat, 
				sum(r/(beta_hat+c(0:(max-1)))) + sum(weight)*log(pi2_hat) )    
    
		diaginv <- c( pi_hat^2/Xj, 
				1/(sum(r/(beta_hat+c(0:(max-1)))^2)-sum(weight)/beta_hat) )
		rank1v <- rep( sqrt(beta_hat*sum(weight))/pi2_hat, (d+1) )
		rank1v[d+1] <- sqrt(sum(weight)/beta_hat)
		newton_iterate <- c(pi_hat, beta_hat) + diaginv*score - 
				sum(diaginv*rank1v*score)/(1+sum(diaginv*rank1v^2))*(diaginv*rank1v)
		...
		check <- all(beta_Newton>0)&&all(pi_Newton>0)&&all(pi_Newton<1)&&
			all(pi2_Newton<1)&&all(pi2_Newton>0)&&
      (!is.nan(beta_Newton))&&(!is.nan(pi_Newton))
		if(check){
			LL_Newton <- sum(r*log(beta_Newton +k))+ sum(Xj*log(pi_Newton))+ 
				N*beta_Newton*log(pi2_Newton)- sum(lgamma(data+1))
			beta_hat <- beta_MM*(LL_MM>=LL_Newton)+beta_Newton*(1-(LL_MM>=LL_Newton))
			pi_hat <- pi_MM*(LL_MM>=LL_Newton)+pi_Newton*(1-(LL_MM>=LL_Newton))
			pi2_hat <- pi2_MM*(LL_MM>=LL_Newton)+pi2_Newton*(1-(LL_MM>=LL_Newton))
		...
		if(display){
			if(!check)
				print(paste("iteration",niter,", MM Algorithm, logL=",LL_MM,sep=""))
			else if( LL_MM >= LL_Newton)
				print(paste("iteration",niter,", MM Algorithm, logL=",LL_MM,sep=""))
			else
				print(paste("iteration",niter,", Newton's Method, logL=",
					LL_Newton, sep=""))
		}
		LL_iter[niter] <- LL2
