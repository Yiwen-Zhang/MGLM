...
Before getting started with model fitting, we introduce two plotting functions that can easily capture the correlation structure of the response counts. 
...
<<label=scatter1, echo=TRUE, pdf=FALSE, results=hide, include=TRUE>>=
scatter.plot(Y, facet=TRUE, free=TRUE)
@

<<label=corr, echo=TRUE, pdf=FALSE, results=hide, include=TRUE>>=
corr.plot(Y)
@
...
Figure \ref{fig:lasso-solpath} displays the trace of BIC along the solution path and the regularized estimate at optimal tuning parameter value.
...
Since the rows of the parameter matrix correspond to predictors, selecting by rows performs variable selection at the predictor level. Figure \ref{fig:group-solpath} displays the trace of BIC along the solution path and the regularized estimate at optimal tuning parameter value.
<<warning=FALSE, cache=FALSE>>=
...
Nuclear norm regularization encourages low rank in the regularized estimate. Figure \ref{fig:nuclear-solpath} displays the trace of BIC along the solution path and the regularized estimate at optimal tuning parameter value.  
